...
struct _FpiDeviceGoodixTlsClass
{
  ...
  const guint8      *psk;      ///< TLS pre-shared key for this device
  guint16            psk_len;
};
...
/**
 * @brief Like goodix_tls_read_image() but with a device specific MCU_GET_IMAGE payload
 * and reply timeout.
 */
void goodix_tls_read_image_payload (FpDevice           *dev,
                                    const guint8       *payload,
                                    guint16             length,
                                    guint               timeout_ms,
                                    GoodixImageCallback callback,
