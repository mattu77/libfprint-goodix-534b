...
err_from_ssl (void)
{
  ...
  return g_error_new (G_IO_ERROR, G_IO_ERROR_FAILED, "openssl: %s",
                                ...
                                unsigned int   max_psk_len)
{
  ...
  if (!self || !self->psk || self->psk_len == 0)
    {
      fp_err ("no PSK configured for this device");
      return 0;
    }
  if (self->psk_len > max_psk_len)
    {
      fp_err ("max psk length (%d) too short (needs %d)", max_psk_len, self->psk_len);
      return 0;
    }
...
tls_server_config_ctx (SSL_CTX *ctx)
{
  ...
  SSL_CTX_set_cipher_list (ctx, "PSK-AES128-CBC-SHA256:ALL:@SECLEVEL=0");
...
tls_config_ssl (SSL *ssl)
{
  ...
  SSL_set_cipher_list (ssl, "PSK-AES128-CBC-SHA256:ALL:@SECLEVEL=0");
}
